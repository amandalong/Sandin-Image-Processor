...
Instead of turning the dials by hand the Oscillator can generate its own signal or drive the video. For example in the configuration: CAMERA + OSC -> COMPARATOR (using both video and osc inputs) -> MONITOR. The modules can be chained to create more complicated array of effects. 
_Comparator module example generated by Oscillation - note the green input_ 
